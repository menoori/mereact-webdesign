...
export class StylesManager {
  ...
  inputBasicStyle = (
    ...
    return {
      borderRadius: ".8rem",
      backgroundImage: this._inputCSS.backgroundImage,
      opacity: disabled ? 0.5 : 1,
      color: disabled
        ? this._inputCSS.firstColor
        : onHover && !noHoverAnimation
        ? this._inputCSS.secondColor
        : this._inputCSS.firstColor,
      boxShadow: disabled
        ? ""
        : onFocus
        ? `0 4px 8px 0 rgba(0,0,0,0.12), inset 0 0 0 2px ${this._inputCSS.firstColor}`
